...
def parse_user_input(user_input):
    ...
    # 2글자 명령어가 아닌 경우
    if len(cmd) != 2:
        print("오류: 명령어는 두 글자여야 합니다 (예: RS, SP).")
        return None, None
    ...
    if cmd in COMMANDS_WITH_DATA:
        if len(parts) < 2:
            print(f"오류: {cmd} 명령어는 데이터가 필요합니다 (예: {cmd} RED).")
            return None, None
        
        color_name = parts[1]
        color_code = COLOR_NAME_MAP.get(color_name)
        if not color_code:
...
def main():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        try:
            ...
            print("사용 가능한 명령어: RS, SS, CS [색상], AS, RE, SP [색상]")
            print("예시: CS RED, SP GREEN")
